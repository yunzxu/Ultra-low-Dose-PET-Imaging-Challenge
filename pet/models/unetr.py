...
class UNETR(nn.Module):
    def __init__(self, img_shape=(96, 96, 96), input_dim=1, output_dim=1, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        super().__init__()
...
class Conv3DBlock_nobn(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3):
        super().__init__()
        self.block = nn.Sequential(
            SingleConv3DBlock(in_planes, out_planes, kernel_size),
...
class Deconv3DBlock_nobn(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3):
        super().__init__()
        self.block = nn.Sequential(
            SingleDeconv3DBlock(in_planes, out_planes),
...
class UNETR_NoBN(nn.Module):
    def __init__(self, img_shape=(96, 96, 96), input_dim=1, output_dim=1, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        ...
        self.decoder0 = \
            nn.Sequential(
                Conv3DBlock_nobn(input_dim, 32, 3),
                Conv3DBlock_nobn(32, 64, 3)
            )

        self.decoder3 = \
            nn.Sequential(
                Deconv3DBlock_nobn(embed_dim, 512),
                Deconv3DBlock_nobn(512, 256),
                Deconv3DBlock_nobn(256, 128)
            )
        ...
        self.decoder9_upsampler = \
            nn.Sequential(
                Conv3DBlock_nobn(1024, 512),
                Conv3DBlock_nobn(512, 512),
                Conv3DBlock_nobn(512, 512),
                SingleDeconv3DBlock(512, 256)
            )
        ...
        self.decoder6_upsampler = \
            nn.Sequential(
                Conv3DBlock_nobn(512, 256),
                Conv3DBlock_nobn(256, 256),
                SingleDeconv3DBlock(256, 128)
            )

        self.decoder3_upsampler = \
            nn.Sequential(
                Conv3DBlock_nobn(256, 128),
                Conv3DBlock_nobn(128, 128),
                SingleDeconv3DBlock(128, 64)
            )

        self.decoder0_header = \
            nn.Sequential(
                Conv3DBlock_nobn(128, 64),
                Conv3DBlock_nobn(64, 64),
                SingleConv3DBlock(64, output_dim, 1)
            )
        # self.outlayer = nn.Sigmoid()
